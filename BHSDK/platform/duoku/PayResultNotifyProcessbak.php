 ...
 */
$AppId = 5179325; //应用开发者appid
$Secretkey = 'pQuTaOpllGcCWGSbvsCUmdpMHWOzL23H';//应用开发者apKey
...
function notify_process($AppId,$Secretkey){
    ...
    HaloLog::addLog('duoku2015', '$UID='.$UID);
    HaloLog::addLog('duoku2015', '$OrderStatus='.$OrderStatus);
    HaloLog::addLog('duoku2015', '$OrderMoney='.$OrderMoney);
    HaloLog::addLog('duoku2015', '$ExtInfo='.$ExtInfo);
    ...
    {
        $newdb = new PlatModel();
        $selectOrderId = $newdb->selectPay($UID,$CooperatorOrderSerial,'platform_duoku_pay');
        //验证是否充值过
        ...
        {
            $sdkPaytime = time();
            $insert = $newdb->insertPay($OrderMoney,'dk2015',$UID,$CooperatorOrderSerial,$ExtInfo,$OrderStatus,$StartDateTime,$sdkPaytime);
            if($insert)
            {
