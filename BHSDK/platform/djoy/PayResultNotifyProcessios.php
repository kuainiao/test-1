...
function payNotifyResult($order, $money, $mid, $time, $result, $ext, $signature)
{
    ...
    if($signature == $localSignature)
    {
        if($result == 1)
        {
            ...
            {
                $sdkPaytime = time();
                $insert = $newdb->insertPay($money, $order, $mid, $time, $ext, $sdkPaytime,$channel_type=2);
                if($insert)
                {
...
//                    die();
					...
					$newUser->insertUserpaylog($mid, $thirdType, $order, $sdkPaytime, $area, $ext, $money, $accountId,2);
                    echo "success";
                }
                else
                {
