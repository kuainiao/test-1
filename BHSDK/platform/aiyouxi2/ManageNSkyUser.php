...
require_once '../../models/NSkyToGameServerModel.php';
require_once '../../models/UserLoginModel.php';

class ManageNSky
{
    public function manageNSkyUser($area, $uid,$server,$systemType=1)
    {
        //定义第三方平台类型
        $third_type = 9;
		
		$server_table = 'platform_nsky_server';
        //查询网站平台uid
        $newDb = new NSkyModel();
        $dbUid = $newDb->selectNSkyUser($area, $uid);

        //查询游戏数据库uid
        $newGameDb = new NSkyToGameServerModel($area);
        $gameDbUid = $newGameDb->selectNSkyUserOnGameServer($uid, $third_type);
        ...
        if ($uid == $dbUid)
        {
            ...
            if ($uid == $gameDbUid)
            {
                ...
                $updateCode = $newGameDb->updateNSkyUserCodeOnGameServer($code, $uid);

                //插入登录用户登录时间
                $account_id = $newDb->selectNSkyUserAccountId($area, $uid);
                ...
                if ($updateCode)
                {
	            	...
	            	if($server!=''){
						...
						if(!$cnt){
							//查看该用户所在大区对应的id
							$area_id = $newDb->selectAreaId('platform_nsky_user', $area, $uid);
							if($area_id){
        ...
        else
        {
        	...
        	$db_username = $newDb->selectNSkyUserCheck($uid);
            //创建游戏用户名、密码
            if($db_username!=''){
            	$wz_username = $db_username;
            }else{
            	$string = substr(md5(time()),8,2);
            	$wz_username = 'nsky'.$this->createRandString(6).$string;
            }
            $wz_password = $this->createRandString(6);
            $md5_wz_password = md5($wz_password);

            //插入游戏服务器username、password
            $insertGameServerUser = $newGameDb->insertNSkyUserOnGameServer($wz_username, $md5_wz_password);

            if($insertGameServerUser)
            {
                $account_id = $newGameDb->selectNSkyUserIdOnGameServer($wz_username);
				if($account_id)
                //插入网站平台uid、account_id、username
                $insertUser = $newDb->insertNSkyUser($area, $uid, $account_id, $wz_username,$systemType);
                ...
                if($insertUser)
                {
                    ...
                    //插入游戏数据库第三方表code
                    $code = $this->createRandString(12);
                    $insertCode = $newGameDb->insertNSkyUserCodeOnGameServer($account_id, $code, $third_type, $uid);
