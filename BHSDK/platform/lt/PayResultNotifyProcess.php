...
$Res = pay_result_notify_process($myGameId,$myGameKey);
...
function pay_result_notify_process($myGameId,$myGameKey)
{
	...
	if(empty($_GET)||!isset($_GET['gameId'])||!isset($_GET['uid'])||!isset($_GET['cpInfo'])||!isset($_GET['itemName'])
        ||!isset($_GET['orderMoney'])||!isset($_GET['orderId'])||!isset($_GET['orderCreateTime'])||!isset($_GET['payStatus'])||!isset($_GET['sign']))
    {
        ...
        if ($payStatus == 1)
        {
            ...
            else
            {
                ...
                if($insert)
                {
                    ...
                    $accountId = $newDb->selectNSkyUserAccountId($area, $uid);
                    $username = $newDb->selectNSkyUserWzUsername($area, $uid);
