...
{
    if(isset($_GET['oldUsername']))//是否存在参数
    {
        $oldUsername = $_GET['oldUsername'];//存在
    }
    else
    {
        echo constant('GET_NO_OLD_USERNAME');
        exit;
    }
    if(isset($_GET['oldUserpass']))//是否存在参数
    {
        $oldUserpass = $_GET['oldUserpass'];//存在
    }
    else
    {
        echo constant('GET_NO_OLD_USERPASS');
        exit;
    }
    if(isset($_GET['username']))//是否存在参数
    {
...
if(preg_match($pcreUsername, $username))
{
    ...
    {
        $new = new Register();
        $return = $new->UserRegister($oldUsername, $oldUserpass, $username, $userpass, $gameId, $systemType, $partner, 'bind');
    }
    else
    {
        echo constant('GET_NO_USERPASS');
