...
function pay_result_notify_process($myGameId,$myGameKey)
{
	...
	if(empty($_GET)||!isset($_GET['gameId'])||!isset($_GET['uid'])||!isset($_GET['cpInfo'])||!isset($_GET['itemName'])
        ...
        ||!isset($_GET['channel']) || !isset($_GET['payStatus'])||!isset($_GET['sign']))
    {
    ...
    $sign_check = md5($gameId.$uid.$itemName.$orderMoney.$orderId.$cpInfo.$orderCreateTime.$payTime.$payStatus.$channel.$myGameKey);
    ...
    {
		$Result["ErrorCode"] =  "14";//注意这里的错误码一定要是字符串格式
