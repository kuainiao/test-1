...
require_once '../../models/WZTJToGameServerModel.php';
...
class ManageThreeJoy
{
    public function manageThreeJoyUser($area,$uid,$server,$systemType=1)
    {
		...
		$server_table = 'platform_wzthreejoy_server';
        //查询网站平台uid
        $newDb = new WZTJModel();
        $dbUid = $newDb->selectThreeJoyUser($area,$uid);
		
        //查询游戏数据库uid
        $newGameDb = new WZTJToGameServerModel($area);
            ...
            if ($uid == $gameDbUid)
            {
                ...
                if ($updateCode)
                {
	            	...
	            	if($server!=''){
						...
						if(!$cnt){
							//查看该用户所在大区对应的id
							$area_id = $newDb->selectAreaId('platform_wzthreejoy_user', $area, $uid);
							if($area_id){
        ...
        else
        {
            ...
            if($db_username!=''){
            	...
            	$wz_username = 'threej'.$this->createRandString(6).$string;
            }
