 ...
 function pay_result_notify_process($request)
    {
		...
		$sign              =  $_GET['sign'];
		$app_secret   = 'cbbd37d0317211a24f414be276f53f9e';
		$app_key        = '32fa6b2b94983437648323bbb7231ac6';
        ...
        $newdb = new PlatModel();
        $selectOrderId = $newdb->selectQhPay($request['user_id'],$request['order_id'],'platform_qh_pay');
        //验证是否充值过
        ...
        {
        	$sdkPaytime = time();
            $insert = $newdb->insertQhPay($request,$app_ext,$sign_return,$sign,$sdkPaytime);
            if($insert)
            {
                $area = Ext::getInstance()->getArea($app_ext);
                $gameMoney = $request['amount'] /10;
                $accountId = $newdb->selectThirdUserAccountId('platform_qh_user',$area,$request['user_id']);
                $username = $newdb->selectThirdUserWzUsername('platform_qh_user',$area,$request['user_id']);
