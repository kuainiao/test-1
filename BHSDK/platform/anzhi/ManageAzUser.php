...
class ManageAz
{
    public function manageAzUser($area,$uid,$server,$channel_type=1)
    {
    	$uid = abs(crc32($uid));//因为其uid为字符串所以转码
        //定义第三方平台类型
        $third_type = 5;
		
		$server_table = 'platform_anzhi_server';
        //查询网站平台Uin
        $newdb = new PlatModel();
		$table = 'platform_anzhi_user';
        ...
        else
        {
            ...
            if($db_username!=''){
            	...
            	$wz_username = 'anzhi'.$this->createRandString(6).$string;
			}
