...
class ManageP
{
    public function managePUser($area,$uid,$server,$channel_type=1)
    {
        //定义第三方平台类型
        $third_type = 8;
		
		$server_table = 'platform_pp_server';
        //查询网站平台Uin
        $newdb = new PlatModel();
		$table = 'platform_pp_user';
        ...
        else
        {
            ...
            if($db_username!=''){
            	...
            	$wz_username = 'ppzs'.$this->createRandString(6).$string;
			}
            ...
            if($insertGameServerUser)
            {
                ...
                $time = time();
                $insertUser = $newdb->insertThirdUser($table,$area,$uid,$account_id,$wz_username,$channel_type=2);
