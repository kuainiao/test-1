...
require_once '../../models/XmToGameServerModel.php';
require_once '../../models/UserLoginModel.php';

class ManageXm
{
    public function manageXmUser($area, $uid,$server,$channel_type=1)
    {
        //定义第三方平台类型
        $third_type = 3;

        $table = 'platform_xiaomi_user';
		$server_table = 'platform_xiaomi_server';
        //查询网站平台Uin
        $newdb = new XmModel();
        $dbUin = $newdb->selectXmUser($area, $uid);

        //查询游戏数据库Uin
        $newgamedb = new XmToGameServerModel($area);
        $gameDbUin = $newgamedb->selectXmUserOnGameServer($uid);
        ...
        if ($uid == $dbUin)
        {
            ...
            if ($uid == $gameDbUin)
            {
                ...
                $updateCode = $newgamedb->updateXmUserCodeOnGameServer($code, $uid);

                //插入登录用户登录时间
                $account_id = $newdb->selectXmUserAccountId($area, $uid);
        ...
        else
        {
        	...
        	$db_username = $newdb->selectXmUserCheck($uid);
            //创建游戏用户名、密码
            if($db_username!=''){
				$wz_username = $db_username;
			}else{
				$string = substr(md5(time()),8,2);
				$wz_username = 'xm'.$this->createRandString(6).$string;
			}
            $wz_password = $this->createRandString(6);
            $md5_wz_password = md5($wz_password);

            //插入游戏服务器username、password
            $insertGameServerUser = $newgamedb->insertXmUserOnGameServer($wz_username, $md5_wz_password);

            if($insertGameServerUser)
            {
                $account_id = $newgamedb->selectXmUserIdOnGameServer($wz_username);
				if($account_id)
                //插入网站平台uin、account_id、username
                $insertUser = $newdb->insertXmUser($area, $uid, $account_id, $wz_username,$channel_type);

                if($insertUser)
                {
                    //插入游戏数据库第三方表code
                    $code = $this->createRandString(12);
                    $insertCode = $newgamedb->insertXmUserCodeOnGameServer($account_id, $code, $third_type, $uid);
