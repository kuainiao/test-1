...
class ManageHw
{
    public function manageHwUser($area,$uid,$server,$channel_type=1)
    {
        //定义第三方平台类型
        file_put_contents('/tmp/adf','success');
        $third_type = 23;
		
		$server_table = 'platform_huawei_server';
        //查询网站平台Uin
        $newdb = new PlatModel();
		$table = 'platform_huawei_user';
        $dbUin = $newdb->selectThirdUser($table,$area,$uid);
        file_put_contents('/tmp/nhgjgghj',$dbUin);
        //查询游戏数据库Uin
        $newgamedb = new PlatToGameServerModel($area);

        $gameDbUin = $newgamedb->selectThirdUserOnGameServer($uid,$third_type);

        file_put_contents('/tmp/ba1535',$gameDbUin.'_'.$dbUin);
        //判断用户是否登录过
        if ($uid == $dbUin)
        {
            //判断用户是否在游戏数据库里
            ...
            {	
                ...
                file_put_contents('/tmp/llllsdf',$gameDbUin.'_'.$updateCode.'_'.$code);
                //插入登录用户登录时间
                ...
                if ($updateCode)
                {
                    ...
                    file_put_contents('/tmp/bfgfg',$cnt.'_'.$code);
                    echo $code;
                }
                else
                {
        ...
        else
        {
            ...
            if($db_username!=''){
            	...
            	$wz_username = 'hw'.$this->createRandString(6).$string;
			}
