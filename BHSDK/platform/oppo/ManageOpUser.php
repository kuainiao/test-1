...
class ManageOp
{
    public function manageOpUser($area,$uid,$server,$channel_type=1)
    {
        //定义第三方平台类型
        $third_type = 13;
		
		$server_table = 'platform_oppo_server';
        //查询网站平台Uin
        $newdb = new PlatModel();
		$table = 'platform_oppo_user';
        ...
        if ($uid == $dbUin)
        {
            ...
            if ($uid == $gameDbUin)
            {
                ...
                if ($updateCode)
                {
					...
					file_put_contents('/tmp/re111',$code . '_' . $uid);
                    echo $code . '_' . $uid;
        ...
        else
        {
            ...
            if($db_username!=''){
            	...
            	$wz_username = 'oppo'.$this->createRandString(6).$string;
			}
