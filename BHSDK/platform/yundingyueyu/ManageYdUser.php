...
class ManageYd
{
    public function manageYdUser($area,$uid,$server,$channel_type=2)
    {
        //定义第三方平台类型
        $third_type = 29;
		
		$server_table = 'platform_yundingyueyu_server';
        //查询网站平台Uin
        $newdb = new PlatModel();
		$table = 'platform_yundingyueyu_user';
        ...
        else
        {
            ...
            if($db_username!=''){
                ...
                $wz_username = 'yd'.$this->createRandString(6).$string;
            }else{
            	$string = substr(md5(time()),8,2);
            	$wz_username = 'yd'.$this->createRandString(6).$string;
			}
