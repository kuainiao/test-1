...
class TJModel extends Model
{
	...
	{
		$db = $this->db;
		$insert = $db->insertTable('platform_threejoy_user', array('NS_id'=>'', 'NS_area'=>$area, 'NS_uid'=>$uid, 'NS_account_id'=>$account_id, 'NS_wz_username'=>$wz_username,'NS_channel_type'=>$channel_type,'NS_create_time'=>time()));
		return $insert;
	}
    ...
    {
        $db = $this->db;
        $result = $db->getResultsByCondition('platform_threejoy_user', sprintf("NS_area = '%s' and NS_uid = '%s';", $area, $uid), 'NS_uid');
        if(is_array($result))
        {
    ...
    {
        $db = $this->db;
        $result = $db->getResultsByCondition('platform_threejoy_user', sprintf("NS_uid = '%s' LIMIT 1;",$uid), 'NS_wz_username');
        if(is_array($result))
        {
            foreach($result as $row)
            {
    ...
    {
        $db = $this->db;
        $result = $db->getResultsByCondition('platform_threejoy_user', sprintf("NS_area = '%s' and NS_uid = '%s';", $area, $uid), 'NS_account_id');
        if(is_array($result))
        {
            foreach($result as $row)
            {
    ...
    {
        $db = $this->db;
        $result = $db->getResultsByCondition('platform_threejoy_user', sprintf("NS_area = '%s' and NS_uid = '%s';", $area, $uid), 'NS_wz_username');
        if(is_array($result))
        {
            foreach($result as $row)
            {
    ...
    {
        $db = $this->db;
        $result = $db->getResultsByCondition('platform_threejoy_pay', sprintf("NS_uid = '%s' and NS_order_id = '%s';", $uid, $orderId), 'NS_order_id');
        if(is_array($result))
        {
            foreach($result as $row)
            {
    ...
    {
        $db = $this->db;
        $insert = $db->insertTable('platform_threejoy_pay', array('NS_id'=>'',
            'NS_uid'=>$uid,
            'NS_cp_info'=>$cpInfo,
