...
class pmmodel {
	...
	function getpmbypmid($uid, $pmid) {
		...
		$pm = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."pm_indexes i LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=i.plid WHERE i.pmid='$pmid'");
                //echo "SELECT * FROM ".UC_DBTABLEPRE."pm_indexes i LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=i.plid WHERE i.pmid='$pmid'";
		if($this->isprivilege($pm['plid'], $uid)) {
			$pms = $this->db->fetch_all("SELECT t.*, p.*, t.authorid as founderuid, t.dateline as founddateline FROM ".UC_DBTABLEPRE.$this->getposttablename($pm['plid'])." p LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=p.plid WHERE p.plid='$pm[pmid]'");
                        //echo "SELECT t.*, p.*, t.authorid as founderuid, t.dateline as founddateline FROM ".UC_DBTABLEPRE.$this->getposttablename($pm['plid'])." p LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=p.plid WHERE p.pmid='$pm[pmid]'";
       ...
       function getpmgroup() {
		$arr = array();
		$pms = $this->db->fetch_all("SELECT * FROM pfbbs.pf_common_grouppm order by id desc");
                $arr = $this->getpostlist($pms);
		return $arr;
	}
	...
	function getpmnum($uid, $type = 0, $isnew = 0) {
		...
		if(!$type) {
                        //echo "SELECT COUNT(*) FROM ".UC_DBTABLEPRE."pm_members m WHERE m.uid='$uid' $newsql";
			$newnum = $this->db->result_first("SELECT COUNT(*) FROM ".UC_DBTABLEPRE."pm_members m WHERE m.uid='$uid' $newsql");
		} else {
                        //echo "SELECT COUNT(*) FROM ".UC_DBTABLEPRE."pm_members m LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=m.plid WHERE m.uid='$uid' $newsql AND t.pmtype='$type'";
	...
	function getpmlist($uid, $filter, $start, $ppp = 10) {
		...
		*/
			...
			$addsql = '';
                ...
                //echo "SELECT * FROM ".UC_DBTABLEPRE."pm_members m LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=m.plid left join `pfbbs`.pf_common_grouppm g on g.authorid = t.authorid WHERE $addsql m.uid='$uid'  ORDER BY m.lastdateline DESC LIMIT $start, $ppp";
        ...
        //获取群组消息
        function getgrouppmlist($uid, $filter, $start, $ppp = 10) {
		if(!$uid) {
			return 0;
		}
		$members = $touidarr = $tousernamearr = array();
		...
		*/
			...
			$addsql = '';
                ...
                //echo "SELECT * FROM ".UC_DBTABLEPRE."pm_members m LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=m.plid left join `pfbbs`.pf_common_grouppm g on g.authorid = t.authorid WHERE $addsql m.uid='$uid'  ORDER BY m.lastdateline DESC LIMIT $start, $ppp";
		$query = $this->db->query("SELECT t.*,m.*,g.message as groupmsg FROM ".UC_DBTABLEPRE."pm_members m LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=m.plid left join `pfbbs`.pf_common_grouppm g on g.authorid = t.authorid WHERE $addsql m.uid='$uid'  ORDER BY m.lastdateline DESC LIMIT $start, $ppp");
                while($member = $this->db->fetch_array($query)) {
			if($member['pmtype'] == 1) {
				$users = explode('_', $member['min_max']);
				$member['touid'] = $users[0] == $uid ? $users[1] : $users[0];
	...
	function getplidbytouid($uid, $touid) {
		if(!$uid || !$touid) {
			return 0;
                ...
                //echo "SELECT plid FROM ".UC_DBTABLEPRE."pm_lists WHERE min_max='".$this->relationship($uid, $touid)."'";
