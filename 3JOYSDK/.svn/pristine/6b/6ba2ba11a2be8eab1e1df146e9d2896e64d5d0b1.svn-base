<?php
require_once dirname(__FILE__).'/'.'./HaloLog.php';
...
// 				echo '<hr/>';
    ...
    public function updateTable($table,$data,$condition)
    {
        ...
        {
            $sql = sprintf('UPDATE sdk_download SET NS_count = NS_count + 1');
            //echo $sql;
            return $this->query($sql);
        }
        return false;
    ...
    public function updateMoney($lightCoin, $money, $uid)
    {
        $sql = sprintf('UPDATE pf_user_money SET light_coin=light_coin+%s,money=money+%s,last_money_time=%s WHERE uid = %s',$lightCoin, $money, time(), $uid);
        //echo $sql;
        return $this->query($sql);
    }

    public function updateTables($table,$data,$condition)
    {
        if(is_array($data))
        {
            $list = array();
