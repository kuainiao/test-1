...
class UserModel extends Model
{
    ...
    public function insertUserInfo($uid, $username, $userpass, $salt)
    {
        $table='sdk_user';
        $db = $this->db;
        $insert = $db->insertTable($table, array('NS_id'=>'', 'NS_uid'=>$uid, 'NS_username'=>$username, 'NS_userpass'=>$userpass, 'NS_salt'=>$salt, 'NS_create_time'=>time()));
        return $insert;
    }

    public function selectUserInfo($username,$action)
    {
        $table='sdk_user';
        $db = $this->db;
        $result = $db->getResultsByCondition($table,sprintf("NS_username = '%s';",$username));
        if(is_array($result))
        {
    ...
    public function updateUsernameUserpass($oldUsername, $oldUserpass, $username, $userpass)
    {
        $table='sdk_user';
        $db = $this->db;
        $update = $db->updateTable($table, array('NS_username'=>$username, 'NS_userpass'=>$userpass), "NS_username='".$oldUsername."' AND NS_userpass='".$oldUserpass."'");
        return $update;
    }
