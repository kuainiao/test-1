	...
	if( ! empty($doc->getElementsByTagName( "notify" )->item(0)->nodeValue) ) {
		...
		if($trade_status == 'TRADE_FINISHED' || $trade_status == 'TRADE_SUCCESS') {
            ...
            else
            {
                ...
                $partner = $new->selectAliPayOrderInfo($out_trade_no, 'partner');
                $createTime = $new->selectAliPayOrderInfo($out_trade_no, 'createTime');
                ...
                if(!empty($ResultArray))
                {
                    if($ResultArray['ErrorCode']==11)
                    {
                        ...
                        if($partner=='cjgh01' || $partner=='cjgh02' || $partner=='cjgh03' || $partner=='cjgh04' || $partner=='cjgh05' || $partner=='cjgh06' || $partner=='cjgh07' || $partner=='cjgh08' || $partner=='cjgh09' || $partner=='cjgh10' || $partner=='cjgh11' || $partner=='cjgh12' ||
                            ...
                            $url = "http://pfsdk.9sky.me/order.php?pass=sfjklipognnfdjnjflgsndllgndsjh&type=alipay&partner=$partner&uid=$uid&orderId=$orderId&orderMoney=$orderMoneyStart&cpInfo=$cpInfo&createTime=$createTime&payTime=$payTime";
                    ...
                    {
                        $updateOrder = $new->updateAliPayOrderStatus($out_trade_no, $trade_no, $ResultArray['ErrorCode'], $buyer_email, $payTime);
                    }
                }
                else
                {
                    $updateOrder = $new->updateAliPayOrderStatus($out_trade_no, $trade_no, '', $buyer_email, $payTime);
                }
                echo "success";
            }
		}
