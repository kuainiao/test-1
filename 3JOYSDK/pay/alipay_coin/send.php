...
class SendAlipay
{
    public function sendToAlipay($uid, $orderMoney)
    {
        //config
        $alipay_config = array();
        //↓↓↓↓↓↓↓↓↓↓请在这里配置您的基本信息↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
        ...
        $req_id = date('Ymdhis');
        //服务器异步通知页面路径
        $notify_url = "http://sdk.3joy.cn/pay/alipay_coin/notify_url.php";
        //页面跳转同步通知页面路径
        $call_back_url = "http://sdk.3joy.cn/pay/alipay_coin/call_back_url.php";
        //操作中断返回地址
        $merchant_url = "http://sdk.3joy.cn/pay/alipay_coin/call_back_url.php";
        //卖家支付宝帐户   公司-》16669667@qq.com，合作-》193977005@qq.com
        $seller_email = 'yangdongjie@9sky.me';
        //商户订单号
        $today = date("Ymd");
        $randNum = rand(1000, 9999);
        $out_trade_no = "$today"."$uid"."$randNum".time();//订单号
        //订单名称
        $subject = '闪币';
        //付款金额
...
/*        para_html_text($para_html_text);
        ...
        {
            $new = new PFUserModel();
            $insertPay = $new->insertAliPayOrder($uid,$out_trade_no,$orderMoney);
        }
        else
        {
            echo 'error';
