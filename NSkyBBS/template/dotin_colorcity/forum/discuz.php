<?php echo 'Theme by Dotin  http://www.dotin1991.net';exit;?>
...
<!--[diy=dotin1]-->
<div id="dotin1" class="area"></div>
<!--[/diy]--></ul></div>
<script type="text/javascript">
jQuery(document).ready(function(){
jQuery('.full').hover(function(){
jQuery(".picinfo", this).stop().animate({height:'60px'},{queue:false,duration:200});
}, function() {
jQuery(".picinfo", this).stop().animate({height:'30px'},{queue:false,duration:200});
});
});
</script>
		...
		<!--{loop $catlist $key $cat}-->
			...
			<div class="bm bmw {if $cat['forumcolumns']} flg{/if} cl">
				<div class="bmm cl">
					...
					<!--{eval $caturl = !empty($cat['domain']) && !empty($_G['setting']['domain']['root']['forum']) ? 'http://'.$cat['domain'].'.'.$_G['setting']['domain']['root']['forum'] : '';}--><div class="title">
					<h2><a href="{if !empty($caturl)}$caturl{else}forum.php?gid=$cat[fid]{/if}" style="{if $cat[extra][namecolor]}color: {$cat[extra][namecolor]};{/if}">$cat[name]</a></h2></div>
				</div>
