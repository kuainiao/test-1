        ...
        foreach($system_arr as $v1){
            ...
            foreach($channel_arr as $v2){
                ...
                foreach($server_arr as $v3){
					...
					$sql = "select count(distinct game_uid) as number  from {$table} where system={$system} and server={$server} and channel_uid={$channel} and pay_time>={$yesterday_unix} and pay_time<{$today_unix}";
        ...
        foreach($system_arr as $v1){
            ...
            foreach($channel_arr as $v2){
                ...
                foreach($server_arr as $v3){
					...
					echo $system.','.$channel.','.$server.','.$table.'===>';
					$sql = "select sum(money) as money from {$table} where system={$system} and server={$server} and channel_uid={$channel} and pay_time>={$yesterday_unix} and pay_time<{$today_unix}";
					echo $sql;
					$user_arr = $this->payM->query_data($sql);
					var_dump($user_arr);
					foreach($user_arr as $v){
        ...
        foreach($system_arr as $v1){
            ...
            foreach($channel_arr as $v2){
                ...
                foreach($server_arr as $v3){
					...
					echo $system.','.$channel.','.$server.','.$table.'===>';
					 $sql = "select sum(money) as money  from {$table} where system={$system} and server={$server} and channel_uid={$channel}  and pay_time>={$yesterday_unix} and pay_time<{$today_unix}  group by game_uid";
					 $user_arr = $this->payM->query_data($sql);
        ...
        foreach($system_arr as $v1){
            ...
            foreach($channel_arr as $v2){
                ...
                foreach($server_arr as $v3){
					...
					echo $system.','.$channel.','.$server.','.$table.'===>';
					$sql = "select distinct game_uid  from {$table} where system={$system} and server={$server} and channel_uid={$channel} and pay_time>={$yesterday_unix} and pay_time<{$today_unix}";
					$user_arr = $this->payM->query_data($sql);
					
					foreach($user_arr as $k=>$v){
						$sql = "select * from {$table} where system={$system} and server={$server} and channel_uid={$channel} and pay_time<{$yesterday_yes} and game_uid={$v->game_uid}";
			...
			for($i=0;$i<24;$i++){
				...
				foreach($channel_arr as $v2){
					...
					foreach($system_arr as $v1){
					   $system = $v1['number'];
					   echo '<br/>'.$system.','.$channel.','.$server.','.$table.'===>';
					   $sql = "select count(distinct game_uid) as number from {$table} where system={$system} and server={$server} and channel_uid={$channel} and pay_time>={$yesterday_time_1} and pay_time<{$yesterday_time_2}";
						$user_arr = $this->payM->query_data($sql);
						foreach($user_arr as $v){
								$num_arr[]=$v->number;
						}
