...
$(function () {
        $('#container').highcharts({
            ...
            title: {
                text: <?php echo "'".$sys.$chan.$ser.$time."'"; ?>,
                x: -20 //center
            },
          
            xAxis: {
				title:{text:'日期',align: 'high'},
                 categories:  [<?php foreach($last_7day_retention as $v) :?>
            ...
            series: [{
                name: <?php echo "'".$time."'"; ?>,
                   data: [<?php foreach($last_7day_retention as $v) :?>
                       <?php echo $v->retention.',';?>
                       <?php endforeach;?>]
            }]
        });
    });
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-list-alt"></i>用户留存率</h2>
                <div style="float:right; font-size:12px; margin-right:10px;">
                    <select type="text" id="times" name="times" class="lstime">
                        <option value='next' selected>次日留存</option>
                        <option value='three'>三日留存</option>
                        <option value='seven'>七日留存</option>
                    </select>
                    ...
                    <script>
                        $('#submitBtn').click(function(){
                            ...
                            window.location.href="<?php echo site_url('gindex/retention');?>/"+start_date+"/"+stop_date+"/"+server+"/"+channel+"/"+system+"/"+times+"/";
                        });
                    </script>
                </div>
