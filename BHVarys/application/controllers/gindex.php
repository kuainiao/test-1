...
class GIndex extends CI_Controller {
	    ...
	    public function daily_max_online_user($start_date='', $stop_date='', $server='', $channel='', $system='')
    {
		...
		public function daily_average_online_user($start_date='', $stop_date='', $server='', $flat='', $system='',$area=''){
			
			
			var_dump($_REQUEST);
			//系统
			/*	
			$flat = isset($_POST['flat']) ?  trim($_POST['flat']) : ''; //平台
			$channel = isset($_POST['channel']) ? intval($_POST['channel']) : 0 ; //渠道
			$area = isset($_POST['area']) ? intval($_POST['area']) : 0 ; //大区
			$server = isset($_POST['server']) ? intval($_POST['server']) : 0 ; //游戏服
			$dstart = isset($_POST['start']) ? trim($_POST['start']) : ''; //开始时间
			$dend = isset($_POST['end']) ? trim($_POST['end']) : ''; //结束时间*/
			 header ( "Content-type: text/html; charset=utf-8" );
			date_default_timezone_set("Asia/Shanghai");
		 if(!empty($this->session->all_userdata()['username'])){
            $this->data['system'] = $this->config->item('system');
            $this->data['flat'] = $this->config->item('flats');
            $this->data['server'] = $this->config->item('serverall');
            $this->data['side'] = $this->side();
            $this->data['username'] = $this->session->all_userdata()['username'];
			$this->data['area'] = $this->config->item('area');
			...
			if(empty($server) && empty($flat) && empty($system) && empty($area)){
					$this->data['username'] = $this->session->all_userdata()['username'];
					
					$this->data['channelt'] = $this->config->item('channelt');
					$this->data['servert'] = $this->config->item('servert');
					$this->data['side'] = $this->side();
					...
					while($time<$today){
						$dt = date("Y-m-d",$time);;
						$sql = "select sum(`ol_unum`) as `nums` from `stats_online_flat` where date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."'";
						
						$arr = $this->bhM->query_data_arr($sql);
						
						if(empty($arr[0]['nums'])){
							$re['nums'] = 0;
						}else{
							
							$re['nums'] = ceil($arr[0]['nums']/24);
						}
						$re['id'] = $id;
						$id++;
						$re['date'] = $dt;
					...
					$this->data['ser'] = '所有服务器';
					$this->data['are'] = '所有大服';
					$this->data['last_7day_daily_return'] = $rst;
				
					
				
				}else if(!empty($flat) && $system==0 && $area==0 && $server==0){
					
					$d_st = strtotime($start_date);
					$d_ed = strtotime($stop_date); 
					$fns = $this->config->item('flats');
					$rst = array();
					$id = 1;
					while ($d_st <= $d_ed) {
						$dt = date('Y-m-d',$d_st);
						$sql = "select sum(`ol_unum`) as `nums` from `stats_online_flat` where `ol_flat`='".$flat."' and date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."' group by `ol_flat`";
						...
						$re['date'] = $dt;
						$rst[] = $re;
						$d_st = strtotime('+1 day',$d_st);
					}
					$this->data['sys'] = '所有系统';
					$this->data['chan'] = $fns[$flat].'平台';
					$this->data['ser'] = '所有服务器';
					$this->data['are'] = '所有大服';
					$this->data['last_7day_daily_return'] = $rst;
					...
					$cn = $this->config->item('channelt');
					$rst = array();
					$id = 1;

					while ($d_st <= $d_ed) {
						$dt = date('Y-m-d',$d_st);
						$sql = "select sum(`ol_unum`) as `nums` from `stats_online_channel` where `ol_type`=".$system." and `ol_flat`='".$flat."' and date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."' group by `ol_type`";
					...
					$d_ed = strtotime($stop_date); 
					$fns = $this->config->item('flats');
					$an = $this->config->item('area');
					$rst = array();
					$id = 1;

					while ($d_st <= $d_ed) {
						$dt = date('Y-m-d',$d_st);
						$sql = "select sum(`ol_unum`) as `nums` from `stats_online_area` where `ol_area`=".$area." and `ol_flat`='".$flat."' and date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."' group by `ol_area`";
						$arr = $this->bhM->query_data_arr($sql);
					...
					while ($d_st <= $d_ed) {
						$dt = date('Y-m-d',$d_st);
						if(!empty($flat)){
							$sql = "select sum(`ol_unum`) as `nums` from `stats_online_server` where `ol_server`=".$server." and `ol_flat`='".$flat."' and date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."' group by `ol_server`";
						}else{
						   $sql = "select sum(`ol_unum`) as `nums` from `stats_online_server` where `ol_server`=".$server." and date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."' group by `ol_server`"; 
						}
						$this->dump( $sql);
					...
					$this->data['sys'] = '所有系统';
					$this->data['chan'] = $fns[$flat].'平台';
					$this->data['ser'] = $sn[$server].'服务器';
				
					$this->data['last_7day_daily_return'] = $rst;
					
				}else if(!empty($flat) && $system>0 && $area>0 && $server==0){
					//平台、渠道、大区
					echo $flat.$system.$area.$start_date;
					$d_st = strtotime($start_date);
					$d_ed = strtotime($stop_date);
					$fns = $this->config->item('flats');
					$an = $this->config->item('area');
					...
					while ($d_st <= $d_ed) {
						$dt = date('Y-m-d',$d_st);
						$sql = "select sum(`ol_unum`) as `nums` from `stats_online_fca` where `ol_type`=".$system." and `ol_flat`='".$flat."' and `ol_area`=".$area." and date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."' group by `ol_area`";
					...
					$this->data['last_7day_daily_return'] = $rst;
					var_dump($this->data['last_7day_daily_return']);
					...
					while ($d_st <= $d_ed) {
						$dt = date('Y-m-d',$d_st);
						$sql = "select sum(`ol_unum`) as `nums` from `stats_online_fcs` where `ol_type`=".$system." and `ol_flat`='".$flat."' and `ol_server`=".$server." and date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."' group by `ol_server`";
						$arr = $this->bhM->query_data_arr($sql);
					...
					while ($d_st <= $d_ed) {
						$dt = date('Y-m-d',$d_st);
						$sql = "select sum(`ol_unum`) as `nums` from `stats_online_channel` where `ol_type`=".$system." and  date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."' group by `ol_type`";
						$arr = $this->bhM->query_data_arr($sql);
					...
					while ($d_st <= $d_ed) {
						$dt = date('Y-m-d',$d_st);
						$sql = "select sum(`ol_unum`) as `nums` from `stats_online_fca` where `ol_type`=".$system." and `ol_area`=".$area." and date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."' group by `ol_area`";
						$arr = $this->bhM->query_data_arr($sql);
					...
					while ($d_st <= $d_ed) {
						$dt = date('Y-m-d',$d_st);
						$sql = "select sum(`ol_unum`) as `nums` from `stats_online_fcs` where `ol_type`=".$system." and `ol_server`=".$server." and date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."' group by `ol_server`";
						$arr = $this->bhM->query_data_arr($sql);
						//var_dump($arr);
					...
					while ($d_st <= $d_ed) {
						$dt = date('Y-m-d',$d_st);
						$sql = "select sum(`ol_unum`) as `nums` from `stats_online_area` where `ol_area`=".$area." and  date_format(from_unixtime(`ol_start`),'%Y-%m-%d')='".$dt."' group by `ol_area`";
						$arr = $this->bhM->query_data_arr($sql);
						//var_dump($arr);
		...
		public function getserver(){
			...
			$aid = $_POST['id'];
			$server = $this->config->item('serverall');
			$str = '';
			$str .= '<select name="server" id="server">';
			$str .= ' <option value="0">请选择游戏服</option>';
			$strtemp='';
			if($aid>0){
				$sql = "select server_id from platform_area where area_id={$aid}";
				$arr = $this->bhM->query_data($sql);
				$arr = $this->object_array($arr);
			
				foreach($server as $k=>$v){
					if(in_array($k,explode(',',$arr[0]['server_id'])))
		...
		//平均在线用户
	    public function testdaily_average_online_user($start_date='', $stop_date='', $server='', $channel='', $system='')
    {
        header ( "Content-type: text/html; charset=utf-8" );
		date_default_timezone_set("Asia/Shanghai");
        if(!empty($this->session->all_userdata()['username'])){
