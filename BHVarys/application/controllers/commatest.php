...
class CommaTest extends CI_Controller {
	...
	public function system(){
		...
		foreach ($system_arr as $ck => $cv) {
			...
			$rst = $this->actiongetdata($date,$t,$tn,$ck,0,0);
			//插入数据库
			foreach($rst as $v){
				$sql = "insert into `stats_day_channel` set `dc_flat`=:f,`dc_fname`=:fn, `dc_type`=:tp,`dc_money`=:m,`dc_usernums`=:num,`dc_paynums`=:pnums,`dc_payaddnums`=:pyadnum,`dc_payaddmoney`=:pyadmy,`dc_updaynums`=:upnums,`dc_upthirdnums`=:thirdnums,`dc_upsevennums`=:sevennums,`dc_upfifteennums`=:fifnums,`dc_upthirtynums`=:thirtynums,`dc_login_nums`=:lgnum,`dc_date`=:d,`dc_time`=:t,`dc_upday`=:dayusers,`dc_upthirdusers`=:thirdusers,`dc_upsevenusers`=:sevenusers,`dc_upfifteenusers`=:fifusers,`dc_upthirtyusers`=:thirtyusers";       
				$parm = array(':f'=>$v['flat'],':fn'=>$v['flatname'],':tp'=>$v['channel'],':m'=>$v['money'],':num'=>$v['usernums'],':pnums'=>$v['paynums'],':pyadnum'=>$v['payaddnums'],':pyadmy'=>$v['payaddmoney'],':upnums'=>$v['updaynums'],':thirdnums'=>$v['upthirdnums'],':sevennums'=>$v['upsevennums'],':fifnums'=>$v['upfifteennums'],':thirtynums'=>$v['upthirtynums'],':lgnum'=>$v['loginnums'],':d'=>$date,':t'=>time(),':dayusers'=>$v['updayusers'],':thirdusers'=>$v['upthirdusers'],':sevenusers'=>$v['upsevenusers'],':fifusers'=>$v['upfifteenusers'],':thirtyusers'=>$v['upthirtyusers']);   
	...
	//日新增用户 ,把该area中服里的数据做遍历相加，然后把该大服的数据相加即可，即server对应该platfrom_area中的表相加 ，我自己加个area字段
